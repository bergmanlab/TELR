    ...
    with open(vcf, "r") as input:
        for line in input:
            ...
            with open(subject, "w") as output:
                    try:
                        subprocess.check_output(["samtools", "faidx", merge_contigs, contig_name], stderr=subprocess.DEVNULL)
            ...
            if os.path.isfile(subject):
                with open(seq2contig_out, "a") as output:
                    mm2_output = subprocess.check_output(["minimap2", "-cx", presets_minimap2, "--secondary=no", "-v", "0", subject, query])
...
def find_te(contigs, ref, te_contigs_annotation, family_annotation, te_freq, te_fa, out, sample_name, gap, overlap, presets):
    ...
    # lift over
    report_meta = annotation_liftover(fasta1 = contigs, fasta2 = ref, bed = te_contigs_annotation, sample_name = sample_name, out_dir = out, preset = presets_minimap2, overlap = overlap, gap = gap, flank_len = 500, family_rm = family_annotation, freq = te_freq)
