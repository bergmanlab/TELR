...
def run_wtdbg2(args):
    ...
    if check_exist(contig_layout):
        ...
        if check_exist(consensus):
            ...
            if polish > 0:
                polish_contig(prefix, reads, consensus, consensus_final,
                              thread, presets_minimap2, polish, contig_name)
            else:
...
def polish_contig(prefix, reads, raw_contig, polished_contig, thread, preset, round, contig_name):
    ...
    while True:
        ...
        command = "minimap2 -t " + polish_thread + " -ax " + preset + " -r2k " + \
            raw_contig + " " + reads + " | samtools sort -@" + polish_thread + " > " + bam
        try:
            ...
            return
        command = "samtools view -F0x900 " + bam + " | wtpoa-cns -t " + \
            polish_thread + " -d " + raw_contig + " -i - -fo " + tmp_contig
        try:
            subprocess.run(command, shell=True, timeout=300,
                           stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        except subprocess.TimeoutExpired:
